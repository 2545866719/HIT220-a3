...
# Drawing nodes
for node_id, node in river.nodes.items():
    plt.scatter(node.x, node.y, label=f"Node {node_id}", s=100)

# Drawing Connections
for node_id, node in river.nodes.items():
    for edge in node.edges:
        plt.plot([node.x, edge.x], [node.y, edge.y], linestyle='-', linewidth=2, alpha=0.7, color='gray')

# Add label
for node_id, node in river.nodes.items():
    plt.annotate(f"Node {node_id}", (node.x, node.y), textcoords="offset points", xytext=(0, 10), ha='center')

# Setting the title and label of the diagram
plt.title("River Network Visualization")
plt.xlabel("X")
plt.ylabel("Y")
